...
**Status (26 Sep 2026):**
- Project **`okwan`** (ref `itvgkkwsxyobkwkrrrwg`, eu-west-1) has all twelve migrations applied, and an admin invite is set for the owner's email. The security advisors are clean apart from intentional notes: `asked_questions`, `admin_audit_log` and `admin_invites` are server-only.
